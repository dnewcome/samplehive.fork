    ...
    : wxFrame(NULL, wxID_ANY, "SampleHive", wxDefaultPosition)
{
    ...
    m_TopSplitter = new wxSplitterWindow(this, wxID_ANY, wxDefaultPosition, wxDefaultSize,
    ...
    m_BottomSplitter->SplitVertically(m_Notebook, m_Library);
    ...
    m_TransportControls = new cTransportControls(m_TopPanel, *m_Library->GetListCtrlObject(), *m_MediaCtrl, *m_Timer);
    m_WaveformViewer = new cWaveformViewer(m_TopPanel, *m_Library->GetListCtrlObject(), *m_MediaCtrl, *m_Database);
    ...
    Bind(SampleHive::SH_EVT_UPDATE_WAVEFORM, &MainFrame::OnRecieveWaveformUpdateStatus, this);
    Bind(SampleHive::SH_EVT_CALL_FUNC_PLAY, &MainFrame::OnRecieveCallFunctionPlay, this);

    // Adding widgets to their sizers
    m_MainSizer->Add(m_TopSplitter, 1, wxALL | wxEXPAND, 0);
    ...
    // m_Notebook->SetSizer(m_BottomLeftPanelMainSizer);
    // m_BottomLeftPanelMainSizer->Fit(m_Notebook);
    // m_BottomLeftPanelMainSizer->SetSizeHints(m_Notebook);
...
void MainFrame::OnRecieveLoopPoints(SampleHive::LoopPointsEvent& event)
{
...
void MainFrame::OnRecievePushStatusBarStatus(SampleHive::StatusBarStatusEvent& event)
{
...
void MainFrame::OnRecievePopStatusBarStatus(SampleHive::StatusBarStatusEvent& event)
{
    m_StatusBar->PopStatusText(event.GetPopMessageSection());
}

void MainFrame::OnRecieveSetStatusBarStatus(SampleHive::StatusBarStatusEvent& event)
{
...
void MainFrame::OnRecieveInfoBarStatus(SampleHive::InfoBarMessageEvent& event)
{
...
void MainFrame::OnRecieveTimerStopStatus(SampleHive::TimerEvent& event)
{
    if (m_Timer->IsRunning())
        m_Timer->Stop();
}

void MainFrame::OnRecieveCallFunctionPlay(SampleHive::CallFunctionEvent& event)
{
    Serializer serializer;
    ...
    wxString sample_path = serializer.DeserializeShowFileExtension() ?
        ...
        m_Database->GetSamplePathByFilename(selection.ToStdString());
...
void MainFrame::OnRecieveWaveformUpdateStatus(SampleHive::WaveformUpdateEvent& event)
{
    m_WaveformViewer->ResetDC();
}
