...
void Database::InsertIntoSamples(std::vector<Sample> samples)
{
    try
    {
        ...
        sqlite3_stmt *stmt = nullptr;
        rc = sqlite3_prepare_v2(m_Database, insert.c_str(), insert.size(), &stmt, NULL);
        ...
        for (unsigned int i = 0; i < samples.size(); i++)
        {
            ...
            rc = sqlite3_bind_int(stmt, 1, sample.GetFavorite());
            rc = sqlite3_bind_text(stmt, 2, filename.c_str(), filename.size(), SQLITE_STATIC);
            rc = sqlite3_bind_text(stmt, 3, file_extension.c_str(), file_extension.size(), SQLITE_STATIC);
            rc = sqlite3_bind_text(stmt, 4, sample_pack.c_str(), sample_pack.size(), SQLITE_STATIC);
            rc = sqlite3_bind_text(stmt, 5, type.c_str(), type.size(), SQLITE_STATIC);
            rc = sqlite3_bind_int(stmt, 6, sample.GetChannels());
            rc = sqlite3_bind_int(stmt, 7, sample.GetLength());
            rc = sqlite3_bind_int(stmt, 8, sample.GetSampleRate());
            rc = sqlite3_bind_int(stmt, 9, sample.GetBitrate());
            rc = sqlite3_bind_text(stmt, 10, path.c_str(), path.size(), SQLITE_STATIC);
            rc = sqlite3_bind_int(stmt, 11, sample.GetTrashed());
            rc = sqlite3_bind_text(stmt, 12, hive.c_str(), hive.size(), SQLITE_STATIC);
...
void Database::InsertIntoHives(const std::string &hiveName)
{
    try
    {
        ...
        // rc = sqlite3_bind_int(stmt, 1, sample.GetFavorite());
        rc = sqlite3_bind_text(stmt, 1, hiveName.c_str(), hiveName.size(), SQLITE_STATIC);
...
void Database::UpdateHive(const std::string &hiveOldName, const std::string &hiveNewName)
{
    try
    {
        ...
        sqlite3_stmt *stmt = nullptr;
        rc = sqlite3_prepare_v2(m_Database, update.c_str(), update.size(), &stmt, NULL);

        rc = sqlite3_bind_text(stmt, 1, hiveNewName.c_str(), hiveNewName.size(), SQLITE_STATIC);
        rc = sqlite3_bind_text(stmt, 2, hiveOldName.c_str(), hiveOldName.size(), SQLITE_STATIC);
...
void Database::UpdateSamplePack(const std::string &filename, const std::string &samplePack)
{
    try
    {
        ...
        rc = sqlite3_bind_text(stmt, 1, samplePack.c_str(), samplePack.size(), SQLITE_STATIC);
        rc = sqlite3_bind_text(stmt, 2, filename.c_str(), filename.size(), SQLITE_STATIC);
...
std::string Database::GetSampleType(const std::string &filename)
{
    ...
    try
    {
        ...
        sqlite3_stmt *stmt = nullptr;
        rc = sqlite3_prepare_v2(m_Database, select.c_str(), select.size(), &stmt, NULL);

        rc = sqlite3_bind_text(stmt, 1, filename.c_str(), filename.size(), SQLITE_STATIC);
...
std::string Database::GetHiveByFilename(const std::string &filename)
{
    ...
    try
    {
        ...
        if (sqlite3_step(stmt) == SQLITE_ROW)
        {
            ...
            hive = std::string(reinterpret_cast<const char *>(sqlite3_column_text(stmt, 0)));
        }
...
void Database::RemoveSampleFromDatabase(const std::string &filename)
{
    try
    {
        ...
        sqlite3_stmt *stmt = nullptr;
        rc = sqlite3_prepare_v2(m_Database, remove.c_str(), remove.size(), &stmt, NULL);
...
std::string Database::GetSampleFileExtension(const std::string &filename)
{
    ...
    try
    {
        ...
        {
            wxLogDebug("Record found, fetching..");
            extension = std::string(reinterpret_cast<const char *>(sqlite3_column_text(stmt, 0)));
        }
                                     ...
                                     const std::string &icon_star_filled, const std::string &icon_star_empty)
{
    ...
    try
    {
        ...
        rc = sqlite3_prepare_v2(m_Database, filter.c_str(), filter.size(), &stmt, NULL);
        rc = sqlite3_bind_text(stmt, 1, sampleName.c_str(), sampleName.size(), SQLITE_STATIC);
        ...
        if (rc == SQLITE_OK)
        {
            ...
            {
                wxLogDebug("Record found, fetching..");
                int favorite = sqlite3_column_int(stmt, 0);
                wxString filename = wxString(std::string(reinterpret_cast<const char *>(sqlite3_column_text(stmt, 1))));
                wxString sample_pack = wxString(std::string(reinterpret_cast<const char *>(sqlite3_column_text(stmt, 2))));
                wxString sample_type = std::string(reinterpret_cast<const char *>(sqlite3_column_text(stmt, 3)));
                int channels = sqlite3_column_int(stmt, 4);
                int length = sqlite3_column_int(stmt, 5);
                int sample_rate = sqlite3_column_int(stmt, 6);
                int bitrate = sqlite3_column_int(stmt, 7);
                wxString path = wxString(std::string(reinterpret_cast<const char *>(sqlite3_column_text(stmt, 8))));
...
void Database::LoadHivesDatabase(wxDataViewTreeCtrl &treeCtrl)
{
    try
    {
        ...
        if (rc == SQLITE_OK)
        {
            ...
            {
                wxLogDebug("Record found, fetching..");
                wxString hive = wxString(std::string(reinterpret_cast<const char *>(sqlite3_column_text(stmt, 0))));
...
wxArrayString Database::CheckDuplicates(const wxArrayString &files)
{
    ...
    try
    {
        ...
        sqlite3_stmt *stmt = nullptr;
        throw_on_sqlite3_error(sqlite3_prepare_v2(m_Database, select.c_str(), select.size(), &stmt, NULL));
                                     ...
                                     const std::string &icon_star_filled, const std::string &icon_star_empty)
{
    ...
    try
    {
        ...
        rc = sqlite3_prepare_v2(m_Database, restore.c_str(), restore.size(), &stmt, NULL);
        ...
        if (rc == SQLITE_OK)
        {
            ...
            {
                int favorite = sqlite3_column_int(stmt, 0);
                wxString filename = std::string(reinterpret_cast<const char *>(sqlite3_column_text(stmt, 1)));
                wxString file_extension = std::string(reinterpret_cast<const char *>(sqlite3_column_text(stmt, 2)));
                wxString sample_pack = std::string(reinterpret_cast<const char *>(sqlite3_column_text(stmt, 3)));
                wxString sample_type = std::string(reinterpret_cast<const char *>(sqlite3_column_text(stmt, 4)));
                int channels = sqlite3_column_int(stmt, 5);
                int length = sqlite3_column_int(stmt, 6);
                int sample_rate = sqlite3_column_int(stmt, 7);
                int bitrate = sqlite3_column_int(stmt, 8);
                wxString path = std::string(reinterpret_cast<const char *>(sqlite3_column_text(stmt, 9)));
                int trashed = sqlite3_column_int(stmt, 10);
                wxString hive_name = std::string(reinterpret_cast<const char *>(sqlite3_column_text(stmt, 11)));
...
void Database::open(const std::string &dbPath)
{
    try
    {
        ...
        const auto qry = std::string("pragma journal_mode = WAL; pragma synchronous = normal; pragma temp_store = memory; pragma mmap_size = 30000000000;");

        sqlite3_stmt *stmt = nullptr;
        throw_on_sqlite3_error(sqlite3_prepare_v2(m_Database, qry.c_str(), qry.size(), &stmt, NULL));
        if (SQLITE_ROW != sqlite3_step(stmt))
        {
            wxLogDebug("Error executing query");
        }
        throw_on_sqlite3_error(sqlite3_finalize(stmt));
    }
    catch (const std::exception &exception)
